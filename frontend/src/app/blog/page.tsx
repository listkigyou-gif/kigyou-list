...
export default async function BlogIndexPage({ searchParams }: PageProps) {
  ...
  const collectionSchema = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    "name": "Kigyou-list ブログ・業界分析レポート",
    "description": "日本全国の企業データを活用した最新の業界分析、企業ランキング、および営業アプローチ戦略のレポート一覧です。",
    "url": "https://kigyoulist.com/blog",
    "publisher": {
      "@type": "Organization",
      "name": "Kigyou-list",
      "logo": {
    ...
    "mainEntity": {
      ...
      "itemListElement": posts.map((post, idx) => ({
        ...
        "url": `https://kigyoulist.com/blog/${post.slug}`,
        "name": post.title
      }))
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0D1117] dark:text-slate-100 transition-colors">
      ...
      <section className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white py-16 px-4 sm:px-6 lg:px-8 border-b border-indigo-950">
        <div className="max-w-7xl mx-auto text-center flex flex-col items-center gap-4">
          ...
          <p className="text-base sm:text-lg text-slate-350 max-w-2xl font-medium">
            全国500万社の企業データベースから抽出したインサイト。地域・業界ごとの代表企業データと効果的なアプローチ法を公開。
          </p>
        </div>
      </section>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col gap-6">
        ...
        <nav className="text-xs font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-1.5 flex-wrap" aria-label="Breadcrumb">
          ...
          <span className="text-slate-800 dark:text-slate-200" aria-current="page">ブログ</span>
        </nav>
        ...
        {posts.length === 0 ? (
          <div className="bg-white border border-slate-200 dark:bg-[#1C2128] dark:border-slate-800 rounded-2xl p-16 text-center shadow-sm flex flex-col items-center gap-4 justify-center">
            ...
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">記事が見つかりません</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              現在、新しいレポート記事を作成中です。しばらく経ってから再度アクセスしてください。
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="p-6 sm:p-7 flex flex-col gap-4 flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1 text-slate-450 dark:text-slate-500 text-2xs font-semibold">
                  ...
                  >
                    <span className="text-2xs font-black text-slate-650 dark:text-slate-400 group-hover/link:text-primary dark:group-hover/link:text-secondary transition-colors">
                      記事を読む
                    </span>
